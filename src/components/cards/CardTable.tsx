...
import { CardArraySchema } from "@/schema/CardSchema";
...
import CardFormArray from "@/components/cards/CardFormArray";
...
const CardTable: React.FC = () => {
  ...
  const updateCards = async (payload: CardUpdatePayload) => {
  ...
  // Usage for deleteCardMutation
  const deleteCardMutation = handleMutation(
    deleteCards,
    (data) => {
      return "Card(s) deleted successfully";
    },
    (recordCount, data) => {
  ...
  // Usage for updateCards
  const updateCardsMutation = handleMutation(
    updateCards,
    (data) => {
      return "Card list updated successfully";
    },
    (recordCount, data) => {
      return (
  ...
  //Client Actions
  const handleSubmit = async (values: CardFormikInitialValues) => {
    //The reference is the index of the row
    const CardsToBeSubmitted = values.Cards.filter((item) => item.touched);

    if (CardsToBeSubmitted.length > 0) {
      const payload: CardUpdatePayload = {
        Cards: CardsToBeSubmitted,
      };

      updateCardsMutation(payload);
    }
  };

  useEffect(() => {
    setMounted(true);
    setMutate(deleteCardMutation);
  }, []);
  ...
  return (
    <Formik
      ...
      validationSchema={CardArraySchema}
      validateOnChange={false}
    >
      {(formik) => <CardFormArray formik={formik} />}
    </Formik>
  );
};
