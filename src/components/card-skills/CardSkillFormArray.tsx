...
import { CardSkillColumns } from "@/components/card-skills/CardSkillColumns";
import { CardSkillMultiCreateDeleteDialog } from "@/components/card-skills/CardSkillMultiCreateDeleteDialog";
...
interface CardSkillFormArrayProps {
  formik: FormikProps<{ CardSkills: CardSkillFormikShape[] }>;
}

const CardSkillFormArray: React.FC<CardSkillFormArrayProps> = ({ formik }) => {
  //URL States
  ...
  const { setRecordsToDelete } = useCardSkillDeleteDialog();
  ...
  const dataRowCount = cardSkillData
    ? cardSkillData.pages
        .slice(0, page)
  ...
  const getCurrentData = (page: number) => {
    return [
      ...cardSkillData!.pages[page - 1].rows.map((item, index) => ({
        ...item,
        index,
        touched: false,
      })),
      {
        ...DEFAULT_CARDSKILL,
        index: cardSkillData!.pages[page - 1].rows.length,
      },
    ];
  };
  ...
  const addRow = () => {
    formik.setFieldValue(`CardSkills`, [
      ...formik.values.CardSkills.map((item) => ({ ...item })),
      { ...DEFAULT_CARDSKILL },
    ]);
    setWillFocus(true);
  };
  ...
  const setTouchedRows = (idx: number) => {
    formik.setFieldValue(`CardSkills[${idx}].touched`, true);
  };

  const deleteRow = (idx: number) => {
    const id = formik.values.CardSkills[idx].id;

    if (id) {
      setRecordsToDelete([id.toString()]);
    } else {
      formik.setFieldValue(`CardSkills`, [
        ...formik.values.CardSkills.slice(0, idx),
        ...formik.values.CardSkills.slice(idx + 1),
      ]);
      formik.setErrors({});
      resetRowSelection();
    }
  ...
  const deleteSelectedRows = () => {
    ...
    const deletedIDs = formik.values.CardSkills.filter((_, idx) =>
    ...
    if (deletedIDs.length > 0) {
      ...
      formik.setFieldValue(
        `CardSkills`,
        removeItemsByIndexes(formik.values.CardSkills, indexes)
      );
      formik.setErrors({});
      resetRowSelection();
    }
  ...
  const toggleSelectAllRow = () => {
    if (Object.keys(rowSelection).length === formik.values.CardSkills.length) {
      resetRowSelection();
    } else {
      setRowSelectionToAll(formik.values.CardSkills.length);
    }
  };
  ...
  const cardSkillTable = useReactTable<CardSkillFormikShape>({
    data: formik.values.CardSkills,
    columns: CardSkillColumns,
    state: {
      sorting: sorting,
      rowSelection,
    },
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4">
        <div className="flex items-center gap-4">
          <div className="text-sm">
            {cardSkillTable.getFilteredSelectedRowModel().rows.length} of{" "}
            {cardSkillTable.getFilteredRowModel().rows.length} row(s) selected.
          </div>
          {hasSelected && (
            <Button
              type="button"
        ...
        <div className="border rounded-md">
          <Table>
            <TableHeader>
              {cardSkillTable.getHeaderGroups().map((headerGroup) => (
            ...
            <TableBody>
              {cardSkillTable.getRowModel().rows?.length ? (
                cardSkillTable.getRowModel().rows.map((row) => (
                  <TableRow
                    key={row.id}
                ...
                <TableRow>
                  <TableCell
                    colSpan={CardSkillColumns.length}
      ...
      <CardSkillMultiCreateDeleteDialog />
    </Form>
  );
};

export default CardSkillFormArray;
