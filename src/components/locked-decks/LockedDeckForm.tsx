...
const LockedDeckForm: React.FC<LockedDeckFormProps> = (prop) => {
  ...
  //Zustand variables
  const { isUpdating, setIsUpdating, hasUpdate, setHasUpdate } = useLockedDeckStore((state) => ({
...
LockedDeckCards: values.LockedDeckCards.map((item, index) => ({
      ...
      lockedDeckMutation.mutateAsync(payload).then((data) => {
      if (addNew) {
        goToNewRecord()
      } else {
        if (data.id) {
...
//Replace the id fields of the indexes from the LockedDeckCards
      const newLockedDeckCards = values.LockedDeckCards.map((item, index) => ({
        ...item,
        id: data.LockedDeckCards.find((item) => item.index === index)?.id || item.id,
      })).sort((a, b) => {
        const desc = lockedDeckCardSort.includes("-");
        const field = desc ? lockedDeckCardSort.substring(1) : lockedDeckCardSort;
  ...
  const renderFormik = (formik: FormikProps<LockedDeckFormFormikInitialValues>) => {
