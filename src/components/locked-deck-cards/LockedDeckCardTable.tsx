...
const LockedDeckCardTable: React.FC = () => {
  ...
  const getLockedDeckCards = async ({ pageParam = "" }) => {
    ...
    const { data } = await axiosClient.get<GetLockedDeckCardsResponse>(`locked-deck-cards`, {
      params: axiosParams,
    });
  ...
  const updateLockedDeckCards = async (payload: LockedDeckCardUpdatePayload) => {
  ...
  //Tanstacks
  const { refetch } = useInfiniteQuery(["lockedDeckCards"], getLockedDeckCards, {
...
type MutationData = { recordsCreated?: number; recordsDeleted?: number };
  ...
  const deleteLockedDeckCardMutation = handleMutation(
  ...
  const updateLockedDeckCardsMutation = handleMutation(
  ...
  const handleSubmit = async (values: LockedDeckCardFormikInitialValues) => {
    ...
    const LockedDeckCardsToBeSubmitted = values.LockedDeckCards.filter((item) => item.touched);
