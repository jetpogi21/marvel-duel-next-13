...
interface LockedDeckCardDeleteDialogProps {
  onSuccess?: () => void;
   formik?: any;
}

export function LockedDeckCardDeleteDialog(props: LockedDeckCardDeleteDialogProps) {
  ...
  const [currentData, resetRowSelection, setCurrentData] = useLockedDeckCardStore((state) => [
    state.currentData,
    state.resetRowSelection,
    state.setCurrentData,
  ]);

  const deleteLockedDeckCards = async (payload: LockedDeckCardDeletePayload) => {
  ...
  const { mutate } = useMutation({
    mutationFn: deleteLockedDeckCards,
    onMutate: () => {
  ...
  const mutateLockedDeckCard = () => {
    mutate && mutate({ deletedLockedDeckCards: recordsToDelete });
  };
