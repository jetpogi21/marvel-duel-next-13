...
import { HeroSkillColumns } from "@/components/hero-skills/HeroSkillColumns";
import { HeroSkillMultiCreateDeleteDialog } from "@/components/hero-skills/HeroSkillMultiCreateDeleteDialog";
...
interface HeroSkillFormArrayProps {
  formik: FormikProps<{ HeroSkills: HeroSkillFormikShape[] }>;
}

const HeroSkillFormArray: React.FC<HeroSkillFormArrayProps> = ({ formik }) => {
  //URL States
  ...
  const dataRowCount = heroSkillData
    ? heroSkillData.pages
        .slice(0, page)
  ...
  const getCurrentData = (page: number) => {
    return [
      ...heroSkillData!.pages[page - 1].rows.map((item, index) => ({
        ...item,
        index,
        touched: false,
      })),
      {
        ...DEFAULT_HEROSKILL,
        index: heroSkillData!.pages[page - 1].rows.length,
      },
    ];
  };
  ...
  const heroSkillTable = useReactTable<HeroSkillFormikShape>({
    ...
    meta: {
      ...
      options: {
        ...
        heroList: heroList || [], //Generated by GetRequiredListForTableForm - GetRequiredListForTableForm
      },
    },
  });
  ...
  return (
    ...
    >
      ...
      <HeroSkillMultiCreateDeleteDialog />
    </Form>
  );
};

export default HeroSkillFormArray;
