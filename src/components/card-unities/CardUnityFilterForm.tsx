...
const CardUnityFilterForm: React.FC = () => {
  ...
  let cardUnityList = cardList as CardModel[];
  //Should only get card unity cards
  //@ts-ignore
  cardUnityList = cardUnityList
    ?.filter((card) => card.CardUnityCards.length > 0)
    .map((card) => ({
      id: card.id,
      name: card.name,
    }));
