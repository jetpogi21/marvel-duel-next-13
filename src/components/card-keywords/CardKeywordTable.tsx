...
const CardKeywordTable: React.FC = () => {
  ...
  const deleteCardKeywords = async (payload: CardKeywordDeletePayload) => {
    const { data } = (await axiosClient({
      url: "card-keywords",
      method: "delete",
      data: payload,
  ...
  // Usage for deleteCardKeywordMutation
  const deleteCardKeywordMutation = useHandleMutation(
    deleteCardKeywords,
    (data) => {
      return "Card Keyword(s) deleted successfully";
    },
    (recordCount, data) => {
  ...
  // Usage for updateCardKeywords
  const updateCardKeywordsMutation = useHandleMutation(
    updateCardKeywords,
    (data) => {
      return "Card Keyword list updated successfully";
    },
    (recordCount, data) => {
      return (
  ...
  const handleSubmit = async (values: CardKeywordFormikInitialValues) => {
    ...
    const CardKeywordsToBeSubmitted = values.CardKeywords.filter(
      (item) => item.touched
    );
    ...
    if (CardKeywordsToBeSubmitted.length > 0) {
      ...
      updateCardKeywordsMutation(payload);
    }
  };

  useEffect(() => {
    setMounted(true);
    setMutate(deleteCardKeywordMutation);
  }, []);
