...
import { CardKeywordColumns } from "@/components/card-keywords/CardKeywordColumns";
import { CardKeywordMultiCreateDeleteDialog } from "@/components/card-keywords/CardKeywordMultiCreateDeleteDialog";
...
interface CardKeywordFormArrayProps {
...
const CardKeywordFormArray: React.FC<CardKeywordFormArrayProps> = ({
  ...
  const { setRecordsToDelete } = useCardKeywordDeleteDialog();
  ...
  const dataRowCount = cardKeywordData
    ? cardKeywordData.pages
        .slice(0, page)
  ...
  const getCurrentData = (page: number) => {
    return [
      ...cardKeywordData!.pages[page - 1].rows.map((item, index) => ({
        ...item,
        index,
        touched: false,
      })),
      {
        ...DEFAULT_CARDKEYWORD,
        index: cardKeywordData!.pages[page - 1].rows.length,
      },
    ];
  };
  ...
  const addRow = () => {
    formik.setFieldValue(`CardKeywords`, [
      ...formik.values.CardKeywords.map((item) => ({ ...item })),
      { ...DEFAULT_CARDKEYWORD },
    ]);
    setWillFocus(true);
  };
  ...
  const setTouchedRows = (idx: number) => {
    formik.setFieldValue(`CardKeywords[${idx}].touched`, true);
  };

  const deleteRow = (idx: number) => {
    const id = formik.values.CardKeywords[idx].id;

    if (id) {
      setRecordsToDelete([id.toString()]);
    } else {
      formik.setFieldValue(`CardKeywords`, [
        ...formik.values.CardKeywords.slice(0, idx),
        ...formik.values.CardKeywords.slice(idx + 1),
      ]);
      formik.setErrors({});
      resetRowSelection();
    }
  ...
  const deleteSelectedRows = () => {
    ...
    const deletedIDs = formik.values.CardKeywords.filter((_, idx) =>
    ...
    if (deletedIDs.length > 0) {
      ...
      formik.setFieldValue(
        `CardKeywords`,
        removeItemsByIndexes(formik.values.CardKeywords, indexes)
      );
      formik.setErrors({});
      resetRowSelection();
    }
  ...
  const cardKeywordTable = useReactTable<CardKeywordFormikShape>({
    data: formik.values.CardKeywords,
    columns: CardKeywordColumns,
    state: {
      sorting: sorting,
      rowSelection,
    },
    ...
    >
      <div className="flex flex-col gap-4">
        ...
        <div className="border rounded-md">
          <Table>
            <TableHeader>
              {cardKeywordTable.getHeaderGroups().map((headerGroup) => (
            ...
            <TableBody>
              {cardKeywordTable.getRowModel().rows?.length ? (
                cardKeywordTable.getRowModel().rows.map((row) => (
                  <TableRow
                    key={row.id}
                ...
                <TableRow>
                  <TableCell
                    colSpan={CardKeywordColumns.length}
      ...
      <CardKeywordMultiCreateDeleteDialog />
    </Form>
  );
};

export default CardKeywordFormArray;
